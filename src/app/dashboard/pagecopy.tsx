    ...
    datasets: [
      {
        label: "Total Stock",
        data: [
          20000, 30000, 50000, 64366, 55000, 52000, 60000, 70000, 65000, 60000,
          55000, 58000,
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-12">
          <div className="md:col-span-4">
            <Link href="/dashboard/product" className="block">
              <Card className="flex flex-col items-start p-6 shadow-md transition hover:bg-gray-200">
                <h3 className="text-md font-semibold">Total Stock</h3>
                <p className="text-2xl font-bold">56,837</p>
                <p className="text-sm text-green-600">
                  10.5% Up from past week
                </p>
              </Card>
            </Link>
            <Link href="/dashboard/product" className="mt-4 block">
              <Card className="flex flex-col items-start p-6 shadow-md transition hover:bg-gray-200">
                <h3 className="text-md font-semibold">
                  Total Low-stock Alerts
                </h3>
                <p className="text-2xl font-bold">120</p>
                <p className="text-sm text-red-600">1.3% Down from past week</p>
              </Card>
            </Link>
            {/* Wrap this card with Link to navigate to Report */}
            <Link href="/dashboard/report" className="mt-4 block">
              <Card className="flex flex-col items-start p-6 shadow-md transition hover:bg-gray-200">
                <h3 className="text-md font-semibold">Total Transactions</h3>
                <p className="text-2xl font-bold">600,432</p>
                <p className="text-sm text-green-600">7.3% Up from yesterday</p>
              </Card>
            </Link>
          </div>
          <Card className="flex flex-col items-start p-6 shadow-md md:col-span-8">
            <h3 className="text-md font-semibold">Total Stock Details</h3>
            <div className="h-64 w-full">
              <Line data={chartData} options={chartOptions} />
            </div>
            <p className="mt-2 text-sm">June 2024</p>
          </Card>
        </div>
      </main>
    </div>
